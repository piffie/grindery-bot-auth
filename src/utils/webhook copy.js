...
export const handleNewLinkReward = async (params) => {
  try {
    const db = await Database.getInstance();
    ...
    const referent = await db
      .collection(USERS_TEST_COLLECTION)
      .findOne({ userTelegramID: params.referentUserTelegramID });

    if (!referent) {
      // The referent user is not in the database
      console.log(
        `[${params.referentUserTelegramID}] referent user is not a user.`
      );
      return true;
    }
    ...
    const rewardWallet =
      ...
      (await getPatchWalletAddressFromTgId(params.referentUserTelegramID));
    ...
    if (txReward.data.txHash) {
      ...
      // Add the reward to the "rewards" collection
      await db.collection(REWARDS_TEST_COLLECTION).insertOne({
        userTelegramID: params.referentUserTelegramID,
      ...
      console.log(`[${params.referentUserTelegramID}] signup reward added.`);
