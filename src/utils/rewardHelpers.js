import { REWARDS_COLLECTION } from './constants.js';
...
async function insertRewardDB(db, params) {
  try {
    ...
    if (params.parentTransactionHash)
      set.parentTransactionHash = params.parentTransactionHash;
    if (params.newUserAddress) set.newUserAddress = params.newUserAddress;
    if (params.sponsoredUserTelegramID)
      set.sponsoredUserTelegramID = params.sponsoredUserTelegramID;
...
async function updateRewardDB(db, params) {
  try {
    const $set = {};
