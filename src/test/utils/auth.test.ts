...
describe('isRequired Middleware', async function () {
  ...
  it('Should return 403 if no credentials sent', async function () {
    ...
    expect(res.json.calledOnceWith({ message: 'No credentials sent' })).to.be
      .true;
  ...
  it('Should return 403 for wrong authentication method', async function () {
    ...
    await authModule.isRequired(req, res, next);
    expect(res.status.calledOnceWith(403)).to.be.true;
    expect(res.json.calledOnceWith({ message: 'Wrong authentication method' }))
      .to.be.true;
    expect(next.notCalled).to.be.true;
  });
  ...
  it('Should set userId and workspaceId in locals', async function () {
    ...
    expect(res.locals.userId).to.equal(
      'eip155:1:0x10A2C306cCc87938B1fe3c63DBb1457A9c810df5',
    );
    expect(res.locals.workspaceId).to.equal(undefined);
  });
});
